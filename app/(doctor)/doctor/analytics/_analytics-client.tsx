...
import { type DoctorAnalyticsData } from "./actions";
...
export default function AnalyticsClient({ initialData }: { initialData: DoctorAnalyticsData }) {
  const [period, setPeriod] = useState<Period>("6m");

  const slice     = PERIOD_SLICES[period];
  const chartData = initialData.monthlyPatients.slice(-slice);

  const totalPatients = chartData.reduce((s, m) => s + m.patients,    0);
  const newPatients   = chartData.reduce((s, m) => s + m.newPatients, 0);
  const condTotal     = initialData.conditions.reduce((s, c) => s + c.count, 0);

  const kpiStats = [
    { label: "Avg patients/day",  value: String(initialData.kpi.avgPatientsPerDay),  change: "", up: null as boolean | null, icon: <Users    className="size-3.5" /> },
    { label: "No-show rate",      value: `${initialData.kpi.noShowRate}%`,            change: "", up: null as boolean | null, icon: <Calendar className="size-3.5" /> },
    { label: "Patient retention", value: `${initialData.kpi.patientRetention}%`,      change: "", up: null as boolean | null, icon: <Repeat   className="size-3.5" /> },
    { label: "Total patients",    value: String(initialData.kpi.totalUniquePatients), change: "", up: null as boolean | null, icon: <Activity className="size-3.5" /> },
  ];
    ...
    <div className="space-y-6 px-4 py-7 sm:px-6 lg:px-8">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Practice performance — {PERIOD_LABELS[period]}</p>
        </div>
        ...
        {kpiStats.map((kpi, i) => (
          <div key={i} className="rounded-2xl border border-border bg-card p-4 shadow-sm transition-shadow hover:shadow-md">
            ...
            <div className="mt-1 flex items-center gap-1">
              ...
              <span className={`text-[10px] font-medium ${kpi.up === true ? "text-vault-positive" : kpi.up === false ? "text-vault-negative" : "text-muted-foreground"}`}>{kpi.change}</span>
            </div>
          </div>
        ))}
      </div>
        ...
        <div className="rounded-2xl border border-border bg-card shadow-sm lg:col-span-7">
          <div className="flex flex-wrap items-center justify-between gap-2 border-b border-border/60 px-5 py-4">
            ...
            <div className="flex gap-4 text-[11px] text-muted-foreground">
              <span className="flex items-center gap-1.5"><span className="size-2 rounded-full bg-[#4D9A7F]" />Total ({totalPatients})</span>
              <span className="flex items-center gap-1.5"><span className="size-2 rounded-full bg-[#C4975A]" />New ({newPatients})</span>
            </div>
          </div>
          ...
          {initialData.appointmentTypes.length === 0 ? (
            ...
            <div className="flex flex-col items-center gap-4 p-5 sm:flex-row sm:items-center">
              <div className="h-32 w-32 shrink-0">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={initialData.appointmentTypes} cx="50%" cy="50%" innerRadius={30} outerRadius={56} paddingAngle={3} dataKey="value">
                      {initialData.appointmentTypes.map((entry, i) => <Cell key={i} fill={entry.color} />)}
                    </Pie>
              ...
              <div className="flex-1 space-y-2.5">
                {initialData.appointmentTypes.map((type, i) => (
        ...
        <div className="rounded-2xl border border-border bg-card shadow-sm lg:col-span-5">
          ...
          <div className="px-3 pb-4 pt-4">
            <div className="h-44 w-full">
              {initialData.weeklyLoad.every((d) => d.appts === 0) ? (
                <div className="flex h-full items-center justify-center text-sm text-muted-foreground">No data yet</div>
              ) : (
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={initialData.weeklyLoad} margin={{ top: 4, right: 8, left: -24, bottom: 0 }}>
                    <CartesianGrid strokeDasharray="2 5" stroke="var(--vault-border-subtle)" vertical={false} />
        ...
        <div className="rounded-2xl border border-border bg-card shadow-sm lg:col-span-7">
          ...
          {initialData.conditions.length === 0 ? (
            ...
            <div className="space-y-3 p-5">
              {initialData.conditions.map((item, i) => {
