    ...
    <>
      ...
      <div className="space-y-6 px-4 py-7 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              {activeCount} active
              {refillDueCount > 0 && (
                <span className="ml-2 rounded-full bg-vault-warning-light px-2 py-0.5 text-[10px] font-semibold text-vault-warning">
                  {refillDueCount} refill due
                </span>
              )}
            </p>
          </div>
          <Button className="gap-2 self-start sm:self-auto" style={{ background: "#4D9A7F", color: "white" }} onClick={() => setShowModal(true)}>
        ...
        <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
          <div className="hidden grid-cols-[1fr_120px_120px_90px_90px_110px_72px] gap-3 border-b border-border/60 px-5 py-3 text-[10px] font-semibold uppercase tracking-[0.1em] text-muted-foreground sm:grid">
            ...
            <div className="divide-y divide-border/50">
              {filtered.map((rx) => {
                ...
                return (
                  <div
                    key={rx.id}
                    className={`flex flex-col gap-2 px-5 py-3.5 transition-colors hover:bg-muted/20 sm:grid sm:grid-cols-[1fr_120px_120px_90px_90px_110px_72px] sm:items-center sm:gap-3 ${
                  ...
                  >
                    ...
                    <div className="flex items-center gap-3">
                      <div className={`flex size-9 shrink-0 items-center justify-center rounded-xl text-[11px] font-bold ${
                        rx.status === "completed" ? "bg-muted/50 text-muted-foreground" : "border border-border bg-card text-foreground"
                      }`}>
                        {rx.avatar}
                      </div>
                      <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {rx.refills - rx.refillsUsed} / {rx.refills}
                    </p>
                    {/* Status */}
                    ...
                    <div className="flex items-center gap-1">
                      <button className="flex size-7 items-center justify-center rounded-lg text-muted-foreground transition-colors hover:bg-muted/60 hover:text-foreground" title="Copy">
                        <Copy className="size-3.5" />
                      </button>
                      <button className="flex size-7 items-center justify-center rounded-lg text-muted-foreground transition-colors hover:bg-muted/60 hover:text-foreground" title="Download PDF">
                        <Download className="size-3.5" />
                      </button>
                      <button className="flex size-7 items-center justify-center rounded-lg text-muted-foreground transition-colors hover:bg-muted/60 hover:text-foreground" title="Open">
