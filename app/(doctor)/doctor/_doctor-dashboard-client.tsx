...
export function DoctorDashboardClient({ data, displayName, greeting, dateStr }: Props) {
  const router = useRouter();
  const [queue,      setQueue]      = useState<QueueAppointment[]>(data.queue);
  ...
  function completePatient(id: string) {
    ...
    startTransition(async () => {
      ...
      if (error) router.refresh(); // revert on failure
    });
  }

  // ── Task toggle — single UPDATE, no SELECT round-trip ──
  function toggleTask(id: string, currentDone: boolean) {
    const newDone = !currentDone;
    setTasks((prev) =>
      prev.map((t) => (t.id === id ? { ...t, is_done: newDone } : t))
    );
    startTransition(async () => {
      const { error } = await toggleDoctorTaskAction(id, newDone);
      if (error) router.refresh(); // revert via fresh server data
    });
  }
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              {dateStr}
              {total > 0 && ` — ${seen} of ${total} patients seen today`}
            </p>
          </div>
      ...
      <section>
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          ...
          <StatCard
            label="Seen So Far"
            value={String(seen)}
            subvalue={total > 0 ? `${Math.round((seen / total) * 100)}% done` : undefined}
            icon={<CheckCircle2 className="size-3.5" />}
          />
          <StatCard
            label="Next Patient"
            value={nextAppt ? fmtTime(nextAppt.start_time) : "All seen"}
            subvalue={nextAppt?.patient?.full_name?.split(" ")[0] ?? undefined}
